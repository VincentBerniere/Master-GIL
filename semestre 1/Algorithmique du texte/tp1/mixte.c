...
Trie createTrie(int maxNode) {
	...
	trie->nextNode = 0;
	trie->transitionRoot = malloc(maxNode * sizeof(int));
    trie->transitionOthers = malloc( maxNode * sizeof(List) );
	int l;
	for (l=0; l <= maxNode; l++) {
		trie->transitionOthers[l] = malloc(sizeof(List) );
		trie->transitionOthers[l]->targetNode = 0;
	}
...
void insertInTrie(Trie trie, unsigned char *w) {
	List tmp = trie->transitionOthers[trie->nextNode];
	int trouve = 0;
	...
	if (tmp->targetNode == 0 || tmp->letter != *w) {
		...
		if (tmp->targetNode == 0) {
			trie->transitionOthers[trie->nextNode] = nouvelleTransition;
		} else {
		...
		//printf("--insertion du mot %c dans le trie, Noeud Courant : %d\n", (char)*w, noeudCourant);
		trie->nextNode = trie->transitionOthers[trie->nextNode]->targetNode;
	}
}
